...
const flattenData = (data) => {
  return data.map((item) => ({
    ...
    moons: JSON.stringify(item.moons),
    semimajorAxis: item.semimajorAxis,
    perihelion: item.perihelion,
    aphelion: item.aphelion,
    eccentricity: item.eccentricity,
    inclination: item.inclination,
    mass: JSON.stringify(item.mass),
    vol: JSON.stringify(item.vol),
    density: item.density,
    gravity: item.gravity,
    escape: item.escape,
    meanRadius: item.meanRadius,
    equaRadius: item.equaRadius,
    polarRadius: item.polarRadius,
    flattening: item.flattening,
    dimension: item.dimension,
    sideralOrbit: item.sideralOrbit,
    sideralRotation: item.sideralRotation,
    aroundPlanet: JSON.stringify(item.aroundPlanet),
    discoveredBy: item.discoveredBy,
    discoveryDate: item.discoveryDate,
    alternativeName: item.alternativeName,
    axialTilt: item.axialTilt,
    avgTemp: item.avgTemp,
    mainAnomaly: item.mainAnomaly,
    argPeriapsis: item.argPeriapsis,
    longAscNode: item.longAscNode,
    bodyType: item.bodyType,
    rel: item.rel,
  }));
};

const flattenedBodies = flattenData(bodies);
const fields = Object.keys(flattenedBodies[0]);
const opts = { fields };

try {
  const csv = parse(flattenedBodies, opts);
  fs.writeFileSync("solar.csv", csv);
